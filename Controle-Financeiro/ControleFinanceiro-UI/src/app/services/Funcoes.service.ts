...
pegarTodos(): Observable<Funcao[]>{
    return this.http.get<Funcao[]>(`${environment.URL_API}/funcoes`)
}

pegarFuncaoPeloId(funcaoId: string): Observable<Funcao>{
    const apiUrl = `${environment.URL_API}/funcoes/${funcaoId}`;
    return this.http.get<Funcao>(apiUrl);
}

novaFuncao(funcao: Funcao): Observable<any> {
    return this.http.post<Funcao>(`${environment.URL_API}/funcoes`, funcao, httpOptions);
}
...
filtrarFuncoes(nomeFuncao: string): Observable<Funcao[]>{
    const apiUrl = `${environment.URL_API}/funcoes/FiltrarFuncoes/${nomeFuncao}`;
    return this.http.get<Funcao[]>(apiUrl);
}
